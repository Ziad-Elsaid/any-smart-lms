...
function StudentViewCourseProgressPage() {
  ...
  async function fetchCurrentCourseProgress() {
    try {
      setLoading(true);
      setError(null);
      console.log("Fetching course progress for userId:", auth?.user?._id, "courseId:", id);
      ...
      if (response?.success) {
        ...
        const errorMsg = response?.message || "Failed to load course data";
        setError(errorMsg);
        console.error("Course progress fetch error:", errorMsg, "Full response:", response);
      }
    } catch (err) {
      const errorMsg = `${err?.message || "Error loading course data"}${err?.response?.data?.message ? " - " + err?.response?.data?.message : ""}`;
      setError(errorMsg);
      console.error("Course progress fetch exception:", err);
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            {/* SIDEBAR */}
            <div
              className={`fixed top-[64px] right-0 bottom-0 w-[400px] bg-white border-l border-[#1E4D2B]/20 transition-all duration-300 ${isSideBarOpen ? "translate-x-0" : "translate-x-full"
                }`}
            >
